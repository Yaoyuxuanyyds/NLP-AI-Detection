...
def train_concat(model, train_dataloader, val_dataloader, criterion, optimizer, scheduler, num_epochs, log_iter, device):
    for epoch in range(num_epochs):
        ...
        val_loss, val_acc = evaluate_concat(model, val_dataloader, criterion, device)
...
# Training function with progress printing
def train_BERT(model, train_dataloader, val_dataloader, criterion, optimizer, scheduler, num_epochs, log_iter, device):
    for epoch in range(num_epochs):
        model.train()
        running_loss = 0.0
        all_preds = []
        ...
        # Evaluate the model on the validation set
        val_loss, val_acc = evaluate_BERT(model, val_dataloader, criterion, device)
        logging.info(f"Epoch {epoch + 1}/{num_epochs}, Val Loss: {val_loss:.4f}, Val Accuracy: {val_acc:.4f}")
        print(f"Epoch {epoch + 1}/{num_epochs}, Val Loss: {val_loss:.4f}, Val Accuracy: {val_acc:.4f}")
...
# Evaluation function
def evaluate_concat(model, dataloader, criterion, device):
    model.eval()
    running_loss = 0.0
    all_preds = []
    all_labels = []
